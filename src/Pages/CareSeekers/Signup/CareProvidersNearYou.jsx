...
function CareProvidersNearYou({ 
  ...
  const isAuthComplete = isEmailComplete && isPasswordComplete;
  ...
  const handleInterested = () => {
    if (!isEmailComplete) {
      ...
      // Normal flow - show subscription popup
      setShowPaymentPopup(true);
    }
  };
  return (
    <div className="w-full bg-white min-h-screen relative">
      {/* Blur overlay when auth is not complete */}
      {!isAuthComplete && (
        <div className="absolute inset-0 bg-white bg-opacity-70 backdrop-blur-sm z-10 flex items-center justify-center">
          <div className="text-center p-8 bg-white rounded-2xl shadow-lg border border-gray-200">
            <img src="/girl.svg" alt="Sign up required" className="w-24 h-24 mx-auto mb-4" />
            <h3 className="text-xl font-semibold text-gray-800 mb-2">
              Sign up to view Care Providers
            </h3>
            <p className="text-gray-500 mb-6">
              Complete your email and password to access care providers near you
            </p>
            <div className="space-y-3">
              {!isEmailComplete && (
                <button
                  onClick={onShowEmailPopup}
                  className="w-full bg-[#0093d1] text-white py-2 px-4 rounded-md hover:bg-[#007bb0] transition"
                >
                  Enter Email Address
                </button>
              )}
      ...
      <div className="flex justify-between items-center px-8 pt-8">
        ...
        <div className="flex items-center">
          <span className="text-lg text-blue-500 font-bold">Step 6</span> 
          <span className="ml-2 text-lg text-gray-500"> of 6</span>
        </div>
      </div>
      ...
      {/* Subscribe Popup - only shown when auth is complete */}
      {showPaymentPopup && isAuthComplete && (
      ...
      {selectedPlan && isAuthComplete && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
          <div className="bg-white rounded-2xl shadow-xl w-[400px] max-w-full relative flex flex-col items-center p-8">
            {/* Close Button */}
            <button
              className="absolute top-4 right-4 text-gray-400 hover:text-gray-700 text-xl font-bold z-10"
              onClick={() => { setSelectedPlan(null); setShowPaymentPopup(false); }}
              aria-label="Close"
            >
              ×
            </button>
            ...
            <button className="w-full border border-[#0093d1] text-[#0093d1] py-3 rounded-lg font-semibold text-lg hover:bg-blue-50 transition" onClick={() => { setSelectedPlan(null); setShowPaymentPopup(false); }}>Cancel</button>
          </div>
        </div>
      )}
    </div>
